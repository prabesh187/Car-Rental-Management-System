...
<?php
    ...
    if (!isset($_POST['radio']) || !isset($_POST['radio1']) || !isset($_POST['driver_id_from_dropdown']) || 
        ...
        die("Error: Missing required booking information. Please go back and fill all fields.");
    }
    ...
    if (empty($type) || empty($charge_type) || empty($driver_id)) {
        die("Error: Please select car type, charge type, and driver.");
    }
    ...
    if (mysqli_num_rows($result0) > 0) {
        ...
        die("Error: Selected car is not available or does not exist.");
    }
    ...
    if (mysqli_num_rows($result_driver_check) == 0) {
        die("Error: Selected driver is not available.");
    }
    if($err_date >= 0) { 
    ...
    // Modified query to handle both client-assigned and independent drivers
    $sql4 = "SELECT c.*, d.*, cl.client_name, cl.client_phone, rc.id 
             FROM cars c 
             CROSS JOIN driver d 
    ...
    if ($result4 && mysqli_num_rows($result4) > 0) {
        while($row = mysqli_fetch_assoc($result4)) {
            ...
            // Handle both client-assigned and independent drivers
            $client_name = $row["client_name"] ? $row["client_name"] : "Independent Driver";
            $client_phone = $row["client_phone"] ? $row["client_phone"] : "N/A";
        }
    } else {
        // Fallback: Get car and driver info separately if the join fails
        ...
        $sql_driver = "SELECT d.*, cl.client_name, cl.client_phone 
                       FROM driver d 
                       LEFT JOIN clients cl ON d.client_username = cl.client_username 
                       WHERE d.driver_id = '$driver_id'";
        $result_driver = $conn->query($sql_driver);
        $driver_data = ($result_driver && mysqli_num_rows($result_driver) > 0) ? mysqli_fetch_assoc($result_driver) : null;
        
        $sql_booking = "SELECT id FROM rentedcars WHERE car_id = '$car_id' AND driver_id = '$driver_id' ORDER BY id DESC LIMIT 1";
        $result_booking = $conn->query($sql_booking);
        $booking_data = ($result_booking && mysqli_num_rows($result_booking) > 0) ? mysqli_fetch_assoc($result_booking) : null;
        
        // Set variables for display with null checks
        $id = $booking_data ? $booking_data["id"] : "Unknown";
        $car_name = $car_data ? $car_data["car_name"] : "Unknown Car";
        $car_nameplate = $car_data ? $car_data["car_nameplate"] : "Unknown";
        $driver_name = $driver_data ? $driver_data["driver_name"] : "Unknown Driver";
        $driver_gender = $driver_data ? $driver_data["driver_gender"] : "Unknown";
        $dl_number = $driver_data ? $driver_data["dl_number"] : "Unknown";
        $driver_phone = $driver_data ? $driver_data["driver_phone"] : "Unknown";
        $client_name = ($driver_data && $driver_data["client_name"]) ? $driver_data["client_name"] : "Independent Driver";
        $client_phone = ($driver_data && $driver_data["client_phone"]) ? $driver_data["client_phone"] : "N/A";
    }
